        ...
        if key != 'file_name_mapping':
            ...
            for root, dirs, files in os.walk(directory):
                for file in files:
                    ...
                    if not (
                        file.endswith(('.xes', '.xes.gz'))
                        or (
                            key.lower().find('helpdesk') != -1 and file.endswith('.csv')
                        )
                    ):
                        os.remove(file_path)
...
def rename_files(directory, name_mapping):
    ...
    for old_name, new_name in name_mapping.items():
        # os.getcwd() provides current directory
        old_path = os.path.join(os.getcwd(), directory, old_name)
        # os.getcwd() provides current directory
...
if result:
    for old_path in result:
        # os.getcwd() provides current directory
        new_path = os.path.join(os.getcwd(), directory, 'env_permit.xes.gz')
        os.rename(old_path, new_path)
    ...
    for file_name, file_path in result:
        ...
        if file_name == 'finale.csv':
            ...
            try:
                ...
                print(
                    'Error! Adjust the names of the mandatory attributes for', file_name
                )
                break
            event_log = pm4py.convert_to_event_log(dataframe)
        new_path = os.path.join(foldername, new_file_name)
...
bpic12c_event_log = pm4py.filter_event_attribute_values(
    bpic12_event_log, 'lifecycle:transition', ['COMPLETE'], level='event', retain=True
)
# get only events related to applications
bpic12a_event_log = pm4py.filter_event_attribute_values(
    bpic12_event_log,
    'concept:name',
    [
        'A_PREACCEPTED',
...
bpic12o_event_log = pm4py.filter_event_attribute_values(
    bpic12_event_log,
    'concept:name',
    [
        'O_SENT',
...
bpic12w_event_log = pm4py.filter_event_attribute_values(
    bpic12_event_log,
    'concept:name',
    [
...
bpic12cw_event_log = pm4py.filter_event_attribute_values(
    bpic12w_event_log, 'lifecycle:transition', ['COMPLETE'], level='event', retain=True
)
